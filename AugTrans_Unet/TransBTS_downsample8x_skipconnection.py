...
class BTS(TransformerBTS):
    def __init__(
        ...
        conv_patch_representation=True,
        positional_encoding_type="learned",
        input_size=[32*32*32, 64*64*64, 128*128*128]
    ):
        ...
        # self.scSE4 = scSE(in_channels=self.embedding_dim//8)#####ldw add###
        self.epa4 = TransformerBlock(input_size=input_size[0], hidden_size = self.embedding_dim//8, proj_size=256,num_heads=4) #####ldw add epa###
        ...
        # self.scSE3 = scSE(in_channels=self.embedding_dim // 16)#####ldw add###
        self.epa3 = TransformerBlock(input_size=input_size[1], hidden_size=self.embedding_dim // 16, proj_size=128,num_heads=4) #####ldw add  epa###
        ...
        # self.scSE2 = scSE(in_channels=self.embedding_dim // 32)#####ldw add###
        self.epa2 = TransformerBlock(input_size=input_size[2], hidden_size=self.embedding_dim // 32, proj_size=64,num_heads=4) #####ldw add epa###
    ...
    def decode(self, x1_1, x2_1, x3_1, x, intmd_x, intmd_layers=[1, 2, 3, 4]):
        ...
        # y4 = self.scSE4(y4)  #add scSE_v2
        y4 = self.epa4(y4)  # add epa4
        y4 = self.DeBlock4(y4)

        y3 = self.DeUp3(y4, x2_1)  # (1, 32, 64, 64, 64)
        # y3 = self.scSE3(y3)  #add scSE_v2
        y3 = self.epa3(y3)  # add epa3
        y3 = self.DeBlock3(y3)

        y2 = self.DeUp2(y3, x1_1)  # (1, 16, 128, 128, 128)
        # y2 = self.scSE2(y2)   #add scSE_v2
        # y2 = self.epa2(y2)  # add epa2
        y2 = self.DeBlock2(y2)
...
def TransBTS(dataset='brats', _conv_repr=True, _pe_type="learned"):
    ...
    model = BTS(
        ...
        embedding_dim=512,
        num_heads=8,
        num_layers=4,   ### Transformer layer number
        hidden_dim=4096,
        dropout_rate=0.1,
