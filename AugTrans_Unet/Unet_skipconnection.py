...
import torch
from .EPA import TransformerBlock
...
norm_layer = partial(SynchronizedBatchNorm3d, momentum=3e-4)
...
class Unet(nn.Module):
    def __init__(self, in_channels=4, base_channels=16, num_classes=4):
        ...
        # self.scSE1 = scSE(in_channels=base_channels)  #####ldw add###
        self.EnDown1 = EnDown(in_channels=base_channels, out_channels=base_channels*2)
        self.epa_block1 = TransformerBlock(input_size=(64 * 64 * 64), hidden_size = base_channels*2, proj_size=128, num_heads=4)  #####ldw add###

        self.EnBlock2_1 = EnBlock(in_channels=base_channels*2)
        # self.EnBlock2_2 = EnBlock(in_channels=base_channels*2)
        # self.scSE2 = scSE(in_channels=base_channels*2)  #####ldw add###
        self.EnDown2 = EnDown(in_channels=base_channels*2, out_channels=base_channels*4)
        self.epa_block2 = TransformerBlock(input_size=(32 * 32 * 32), hidden_size=base_channels * 4, proj_size=256, num_heads=4)  #####ldw add###

        self.EnBlock3_1 = EnBlock(in_channels=base_channels * 4)
        # self.EnBlock3_2 = EnBlock(in_channels=base_channels * 4)
        # self.scSE3 = scSE(in_channels=base_channels * 4)  #####ldw add###
        self.EnDown3 = EnDown(in_channels=base_channels*4, out_channels=base_channels*8)
        # self.epa_block3 = TransformerBlock(input_size=[0], hidden_size=base_channels * 8, proj_size=256,num_heads=4)  #####ldw add###
