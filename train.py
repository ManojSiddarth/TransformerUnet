...
# from models.TransBTS.TransBTSUnet_baseline import Unet0
...
# DataSet Information
parser.add_argument('--root', default='/home/user/4TB/datasets', type=str)

parser.add_argument('--train_dir', default='/home/user/liudongwei/datasets/MICCAI_BraTS2020_TrainingData', type=str)

parser.add_argument('--valid_dir', default='/home/user/4TB/datasets/MICCAI_BraTS2020_ValidationData', type=str)

parser.add_argument('--mode', default='train', type=str)

parser.add_argument('--train_file', default='/home/user/liudongwei/datasets/MICCAI_BraTS2020_TrainingData/train.txt', type=str)

parser.add_argument('--valid_file', default='/home/user/4TB/datasetsMICCAI_BraTS2020_ValidationData/valid.txt', type=str)
...
def main_worker():
    wandb.init(project="TransBTS", name='AugTrans_EPA_UP')
    ...
    # model = nn.parallel.DistributedDataParallel(model, device_ids=[0, 1], output_device=args.local_rank,
    #                                             find_unused_parameters=True)

    model = torch.nn.DataParallel(model)  # 全部gpu
    model.train()

    optimizer = torch.optim.AdamW(model.parameters(), lr=args.lr, weight_decay=args.weight_decay, amsgrad=args.amsgrad)

    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer=optimizer, T_max=args.end_epoch)
    ...
    resume = '' #/home/user/liudongwei/BraTs3D/AugTrans_Unet_EPA_upsampe/checkpoint/experiments2023-02-25/model_epoch_199.pth
    ...
    for epoch in range(args.start_epoch, args.end_epoch):
        ...
        scheduler.step() #new add learningrate change
        ...
        for i, data in enumerate(train_loader):
            ...
            if args.local_rank == 0:
                ...
                wandb.log({'reduce_loss': reduce_loss,
                           'reduce_loss1': reduce_loss1,
                           'reduce_loss2': reduce_loss2,
                           'reduce_loss3': reduce_loss3})
